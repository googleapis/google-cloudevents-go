...
func typePath(importPath string, sep string) string {
	...
	return strings.Trim(strings.Join(tokens, sep), "\"")
}
...
func generateTypeUtils(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	for _, msg := range file.Messages {
		...
		g.P("func UnmarshalStrict", msg.GoIdent.GoName, "(datatype string, data []byte) (*", msg.GoIdent.GoName, ", error) {")
		g.P("  out := ", msg.GoIdent.GoName, "{}")
		g.P()
		g.P(`  switch datatype {`)
		g.P(`  case "application/json":`)
	...
	id := TypePrefix + typePath(file.GoImportPath.String(), ".")
...
var sampleTestTpl *template.Template
...
func init() {
	...
	sampleTestTpl = template.Must(template.ParseFiles(filepath.Join(p, "sample.gotpl")))
}
...
func generateTests(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	params := TestParams{
		...
		SrcPath:  typePath(file.GoImportPath.String(), string(filepath.Separator)),
		ProtoPkg: TypePrefix + typePath(file.GoImportPath.String(), "."),
	}

	tokens := strings.Split(params.SrcPath, string(filepath.Separator))
	params.PkgAlias = tokens[len(tokens)-2]
...
// generateSamples generates code samples per type.
func generateSamples(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	var b bytes.Buffer
	if err := sampleTestTpl.Execute(&b, params); err != nil {
		log.Fatal(err)
	}
	g.P("// Code generated by protoc-gen-go-snowpea. DO NOT EDIT.")
	g.P(b.String())
