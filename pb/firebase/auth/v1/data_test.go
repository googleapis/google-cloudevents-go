...
import (
	...
	auth "github.com/googleapis/google-cloudevents-go/pb/firebase/auth/v1"
)
...
func TestParsingAuthEventData(t *testing.T) {
	...
	for name, file := range cases {
		...
		obj, err := auth.UnmarshalAuthEventData("application/json", data)
		if err != nil {
		...
		cleanData, err := cleanAcceptedUnknowns(data, "AuthEventData", auth.CloudEventTypePrefix())
		if err != nil {
			t.Errorf("%s: could not clean data for strict validation: %v", name, err)
		}

		obj, err = auth.UnmarshalStrictAuthEventData("application/json", cleanData)
		if err != nil {
