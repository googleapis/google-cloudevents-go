...
import (
	...
	audit "github.com/googleapis/google-cloudevents-go/pb/cloud/audit/v1"
)
...
func TestParsingLogEntryData(t *testing.T) {
	...
	for name, file := range cases {
		...
		obj, err := audit.UnmarshalLogEntryData("application/json", data)
		if err != nil {
		...
		cleanData, err := cleanAcceptedUnknowns(data, "LogEntryData", audit.CloudEventTypePrefix())
		if err != nil {
			t.Errorf("%s: could not clean data for strict validation: %v", name, err)
		}

		obj, err = audit.UnmarshalStrictLogEntryData("application/json", cleanData)
		if err != nil {
...
func findTestData(t *testing.T, dataType string) map[string]string {
	...
	testData := filepath.Join(testDataRoot, "testdata", "google", "events", "cloud/audit/v1")
