 ...
 */
const marshalUnmarshalFunctions = (dataFieldName) => `
...
async function main() {
    ...
    filePaths.forEach(filePath => {
        ...
        const typeFileWithMarshalling = typeFileContent +
            marshalUnmarshalFunctions(dataField);
        // Add Go package header (but under Apache header)
        const filePackageHeader = `package ${goPackage}

import "encoding/json"`;
        let typeFileWithMarshallingAndPackage = typeFileWithMarshalling.split('\n');
        typeFileWithMarshallingAndPackage.splice(13, // length of Apache header from quicktype
