...
type BuildEventData struct {
	...
	QueueTTL         *string             `json:"queueTtl,omitempty"`         // TTL in queue for this build. If provided and the build is enqueued longer; than this value, the build will expire and the build status will be; `EXPIRED`.; ; The TTL starts ticking from create_time.
	...
	Steps            []BuildStep         `json:"steps,omitempty"`            // The operations to be performed on the workspace.
	...
	Timeout          *string             `json:"timeout,omitempty"`          // Amount of time that this build should be allowed to run, to second; granularity. If this amount of time elapses, work on the build will cease; and the build status will be `TIMEOUT`.
...
type Results struct {
	...
	Images           []BuiltImage    `json:"images,omitempty"`              // Container images that were built as a part of the build.
...
type SourceProvenance struct {
	FileHashes            map[string]FileHashes       `json:"fileHashes,omitempty"`            // Hash(es) of the build source, which can be used to verify that; the original source integrity was maintained in the build. Note that; `FileHashes` will only be populated if `BuildOptions` has requested a; `SourceProvenanceHash`.; ; The keys to this map are file paths used as build source and the values; contain the hash values for those files.; ; If the build source came in a single package such as a gzipped tarfile; (`.tar.gz`), the `FileHash` will be for the single path to that file.
...
type FileHashes struct {
	FileHash []Hash `json:"fileHash,omitempty"` // Collection of file hashes.
}
...
type BuildStep struct {
	...
	Timeout    *string     `json:"timeout,omitempty"`    // Time limit for executing this build step. If not defined, the step has no; time limit and will be allowed to continue to run until either it completes; or the build itself times out.
